<x-app-layout>
    <x-slot name="header">
        <div class="text-[11px] uppercase tracking-wide text-muted-500">{{ __('Roles') }} / {{ __('Edit') }}</div>
    </x-slot>

    <div class="app-form-page">
        <div class="app-form-page__intro">
            <h1 class="app-form-page__title">{{ __('Edit role') }}</h1>
            <p class="app-form-page__sub">
                {{ __('Update the role name and its permissions.') }}
            </p>
        </div>

        @php $assigned = old('permissions', $role->permissions->pluck('name')->all()); @endphp

        <form method="POST" action="{{ route('admin.roles.update', $role) }}" class="app-form">
            @csrf
            @method('put')
